                        ...
                        <div className="w-full flex justify-center items-end gap-1" style={{ height: '100%' }}>
                            {valueKeys.map(key => (
                                <div key={key} className="group relative flex-1" style={{ height: `${(item.values[key] / maxValue) * 100}%` }}>
                                    ...
                                    ></div>
                                    <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 w-max bg-brand-bg-dark text-brand-text-light text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
