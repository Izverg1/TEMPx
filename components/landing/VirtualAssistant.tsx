    ...
    return (
        <div className="fixed bottom-8 right-8 z-50">
            ...
            <div className={`transition-all duration-300 ease-out ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                <div className="absolute bottom-[calc(100%+1rem)] right-0 w-80 sm:w-96 h-[500px] flex flex-col glass-card rounded-lg shadow-2xl shadow-brand-cyan/20">
                    {/* Header */}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[80%] rounded-lg px-3 py-2 text-sm ${
                                    msg.role === 'user' 
                                        ? 'bg-brand-cyan text-brand-bg-dark' 
                                        : 'bg-brand-bg-dark'
                                }`}>
                                    {msg.text}
                                </div>
                        ...
                        {isLoading && (
                            <div className="flex justify-start">
                                 <div className="max-w-[80%] rounded-lg px-3 py-2 text-sm bg-brand-bg-dark flex items-center gap-2">
                                     <span className="w-2 h-2 bg-brand-cyan rounded-full animate-pulse"></span>
                                     <span className="w-2 h-2 bg-brand-cyan rounded-full animate-pulse" style={{ animationDelay: '200ms' }}></span>
                                     <span className="w-2 h-2 bg-brand-cyan rounded-full animate-pulse" style={{ animationDelay: '400ms' }}></span>
                                 </div>
                            </div>
                        )}
            ...
            <Button 
                ...
                size="lg" 
                className={`rounded-full w-16 h-16 shadow-lg shadow-brand-cyan/40 transition-transform duration-300 ${isOpen ? 'scale-0' : 'scale-100'}`}
            >
                <ChatIcon />
            </Button>
        </div>
