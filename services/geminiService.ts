...
export const generateAgentPersona = async (prompt: string): Promise<AgentPersona> => {
    ...
    const fullPrompt = `Based on the following user description, create a detailed persona for a customer service AI agent. 
...
export const getChatResponse = async (prompt: string): Promise<string> => {
    ...
    Your goal is to answer questions about the platform's features (like the AI Agent Foundry, Workflow Intelligence, Nerve Center, QA Simulation), benefits (like cost savings, quality, security), and the industries it serves (Healthcare, Insurance, Finance, etc.). 
